...
open class Machine(val f: Field) {
    ...
    fun travelTo(x: Int, y: Int) {
        val facingSide = this.getFacingSide()
        val pos = this.getPos()
        ...
        if(x == cX && y == cY) {
            cmd.execute("I")
        } else if (facingSide == CardinalHelper.NORTH && x > cX) {
            cmd.execute("D")
            cmd.execute("D")
        ...
        else if (facingSide == CardinalHelper.NORTH && y > cY) {
            cmd.execute("D")
        ...
        else if (facingSide == CardinalHelper.EAST && x > cX) {
            cmd.execute("D")
            for (i in 0 until x - cX) {
                cmd.execute("M")
            }
        ...
        else if (facingSide == CardinalHelper.EAST && y > cY) {
            for (i in 0 until y - cY) {
                cmd.execute("M")
            }
            travelTo(x, y)
        ...
        else if (facingSide == CardinalHelper.WEST && x > cX) {
            cmd.execute("E")
            for (i in 0 until x - cX) {
                cmd.execute("M")
            }
        ...
        else if (facingSide == CardinalHelper.WEST && y > cY) {
            cmd.execute("D")
            cmd.execute("D")
            for (i in 0 until y - cY) {
                cmd.execute("M")
        ...
        else if (facingSide == CardinalHelper.SOUTH && x > cX) {
            for (i in 0 until (x - cX)) {
                cmd.execute("M")
            }
            travelTo(x, y)
        ...
        else if (facingSide == CardinalHelper.SOUTH && y > cY) {
            cmd.execute("E")
            for (i in 0 until y - cY) {
                cmd.execute("M")
            }
    ...
    private fun setFacingSide(pos: CardinalHelper): String {
        return when (pos) {
            CardinalHelper.NORTH -> {
                this.currentFacingSide = "^"; currentFacingSide
            }
            CardinalHelper.SOUTH -> {
                this.currentFacingSide = "v"; currentFacingSide
            }
            CardinalHelper.EAST -> {
                this.currentFacingSide = ">"; currentFacingSide
            }
     ...
     fun getFacingSide(): CardinalHelper {
